...
!
      ...
      character :: char7*11,char8*28,charr*55,str1*46,str2*46,str3,&
...
855      continue
         ...
         count1 = 0
710      read(47,'(2x,a11,a28,a2,a2,i1)')char7,char8,label,&
                                                char33,iorb
         tmpocc = iocc
         if (char7.ne.'') then
!           Debug write
            ...
            if ((label.eq.'GS'.and.iorb.eq.0).or.(label.eq.'GS'.and.&
                 iorb.eq.1)) then
               read(47,'(a5,es15.8)')char9,coef
               write(102,fmt1)coef,&
                       (tmpocc(k),k=1,nspace),'0:',0,'0:',0,'0:',0,&
                       '0:',0
!               write(6,*)iorb,coef
            else if (label.eq.'S'.and.iorb.eq.1) then
               read(47,'(a5,es15.8,a37,2(1x,i3))')char9,coef,char10,&
                                                int1,int2
               if (lfreeze) then
                  int1 = int1 + nfreeze
                  int2 = int2 + nfreeze
...
!c               ncoef(count1) = coef
               if (nci(int2).le.nspace) then
                  tmpocc(nci(int1)) = 1
                  tmpocc(nci(int2)) = 2
                  !write(102,'(es15.8,nspace(1x,i1),4(1x,i2))')coef,&
                  write(102,fmt1)coef,&
                       (tmpocc(k),k=1,nspace),'0:',0,'0:',0,'0:',0,&
                       '0:',0
               else
               ...
               read(47,'(a5,es15.8,a37,4(1x,i3))')char9,coef,char10,&
                                                 int1,int2,int3,int4
               if (lfreeze) then
                  int1 = int1 + nfreeze
                  int2 = int2 + nfreeze
               ...
               read(47,'(a5,es15.8,a37,6(1x,i3))')char9,coef,char10,&
                                                 int1,int2,int3,int4,&
                                                 int5,int6
               if (lfreeze) then
                  int1 = int1 + nfreeze
